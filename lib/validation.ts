...
export function isValidEgyptianPhoneNumber(phoneNumber: string): boolean {
    if (!phoneNumber) return true; // Optional validation - empty values are valid
...
export function isValidEmail(email: string): boolean {
    if (!email) return true; // Optional validation - empty values are valid
...
export const ValidationUtils = {
     ...
     * Validates a phone number (Egyptian format)
     * @param phoneNumber The phone number to validate
     * @param language The language for error messages ('ar' for Arabic, 'en' for English)
     * @returns Object with validity and error message
     */
    validatePhone: (phoneNumber: string, language: 'ar' | 'en' = 'ar') => {
        const isValid = isValidEgyptianPhoneNumber(phoneNumber);
        const errorMessages = {
            ar: 'الرجاء إدخال رقم هاتف مصري صحيح (مثلاً: +201234567890)',
            en: 'Please enter a valid Egyptian phone number (e.g., +201234567890)'
        };
        return {
            isValid,
            errorMessage: isValid ? null : errorMessages[language]
        };
    },
     ...
     * @returns Object with validity and error message
     */
    validateEmail: (email: string, language: 'ar' | 'en' = 'ar') => {
